...
public class Cargo {
	...
	private Cargo() {
		...
		Controls.getInstance().registerDigitalCommand(ControllerID.Joystick2.getId(), ControlsConfig.SPIN_INTAKE, DigitalType.DigitalHold, () -> {
			spinIntake(0.5, 0.5);
		});
		Controls.getInstance().registerDigitalCommand(ControllerID.Joystick2.getId(), ControlsConfig.SPIN_INTAKE, DigitalType.DigitalRelease, this::stopIntake);
		Controls.getInstance().registerDigitalCommand(ControllerID.Joystick2.getId(), ControlsConfig.SPIN_OUTTAKE, DigitalType.DigitalHold, () -> {
			spinOuttake(0.5, 0.5);
		});
		Controls.getInstance().registerDigitalCommand(ControllerID.Joystick2.getId(), ControlsConfig.SPIN_OUTTAKE, DigitalType.DigitalRelease, this::stopIntake);

		Controls.getInstance().registerAnalogCommand(ControllerID.Joystick2.getId(), ControlsConfig.MOVE_ANGLE, AnalogType.OutOfThresholdMinor, value -> {
			if(!manual){
		...
		Controls.getInstance().registerAnalogCommand(ControllerID.Joystick2.getId(), ControlsConfig.MOVE_ANGLE, AnalogType.InThresholdMinor, value -> {
			if(manual){
		...
		Controls.getInstance().registerDigitalCommand(ControllerID.Joystick2.getId(), ControlsConfig.CARGO_HEIGHT, DigitalType.DigitalPress, ()->{
			if(!manual && configEncoder){
				cargoConveyor();
			}
		});
		Controls.getInstance().registerDigitalCommand(ControllerID.Joystick2.getId(), ControlsConfig.ROCKET_HEIGHT, DigitalType.DigitalPress, ()->{
			if(!manual && configEncoder){
				rocketConveyor();
			}
		});
		Controls.getInstance().registerDigitalCommand(ControllerID.Joystick2.getId(), ControlsConfig.GROUND_HEIGHT, DigitalType.DigitalPress, ()->{
			if(!manual && configEncoder){
				groundConveyor();
			}
		});
		Controls.getInstance().registerDigitalCommand(ControllerID.Joystick2.getId(), ControlsConfig.STATION_HEIGHT, DigitalType.DigitalPress, ()->{
			if(!manual && configEncoder){
				stationConveyor();
			}
		});
